    ...
    el-popover(placement='right', trigger="hover", :disabled='!(mode === "inventory" || mode === "templates" || mode === "assets" || mode === "market-sales" || mode === "market-auctions" || mode === "setsList" || mode === "listings" || this.mode === "sold" || this.mode === "auctions" || this.mode === "bought")', :openDelay='500' )
      .popover-panel
        .popover__top-panel.d-flex
          ...
          div(:class="['main-img', 'radius10', (mode === 'setsList' || mode === 'templates' ? 'sets-list-mode' : '')]", v-else, :style='defaultBackground')
          .popover__top-right-panel
            h4.popover-title.mt-0.mb-2(v-if='mode !== "setsList" && mode !== "templates"') {{cardName}}
            h4.popover-title.mt-0.mb-2(v-if='mode === "setsList" || mode === "templates"') Template ID:
            ...
            p.popover-backed-token(v-if='mode !== "setsList" && mode !== "templates"') Backed Tokens:
              span.ml-1 {{backedToken}}
            p.popover-lowest-listing(v-if='mode !== "setsList" && mode !== "templates"') Lowest Listing:
              span.ml-1.gold-text {{ new Intl.NumberFormat().format(lowestListing) }}WAX
              span.usd-price.green-text (${{ $systemToUSD(lowestListing) }})
            p.popover-highest-listing(v-if='mode !== "setsList" && mode !== "templates"') Highest Listing:
              span.ml-1.gold-text {{ new Intl.NumberFormat().format(highestListing) }}WAX
              span.usd-price.green-text (${{ $systemToUSD(highestListing) }})
            p.popover-issued-supply(v-if='mode === "setsList" || mode === "templates"') Issued Supply:
...
export default {
  ...
  computed: {
    ...
    waxPrice() {
      ...
      } else if (this.mode === 'listings' || this.mode === 'bought' || this.mode === 'auctions' || this.mode === 'sold') {
    ...
    lowestListing() {
      if (this.overData && this.overData.saleData[0]) {
        return (this.overData.saleData[0].listing_price /
          Math.pow(10, 8))
      }
      return 0
    },
